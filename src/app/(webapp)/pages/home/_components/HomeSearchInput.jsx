  ...
  return (
    <>
      ...
      <div className="relative m-1 p-1 min-w-[400px]  w-[76%] max-w-[800px] inline-flex flex-row items-center justify-center">
        ...
        {items.length > 0 ? (
          <div className="absolute bg-white w-[90%] border-t-0 border-b  border-l border-r h-auto  z-20  top-12 rounded-b-2xl  max-h-[200px]  overflow-y-auto overscroll-y-auto overflow-x-hidden   m-1 p-1">
            {items.map((item) => {
              {
                return (
                  <div className="p-1" key={nanoid()}>
                    <Link
                      href={`/pages/home/vectors?id=${item.id}`}
