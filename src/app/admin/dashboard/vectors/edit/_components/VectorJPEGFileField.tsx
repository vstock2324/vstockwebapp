...
import { handleChangeVectorJPEGFile } from '@/actions/admin/dashboard/vectors';

const VectorJPEGFileField=()=>{
  const params=useParams<{id:string}>();
   const handleChangeVectorJPEGFileWithVectorId=handleChangeVectorJPEGFile.bind(null,params.id)
  return (
    <form action={handleChangeVectorJPEGFileWithVectorId} className="space-y-4 mt-10 w-[80%]">
        <Label className=" after:ml-0.5 after:text-red-500 after:content-['*'] text-gray-400 my-2 block" htmlFor="vectorfile">
         Change Vector JPEG File 
        </Label>
        <FileInput  name="vectorjpegfile"  sizing="lg" />
...
export default memo(VectorJPEGFileField);